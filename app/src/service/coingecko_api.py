...
class CoinGeckoAPI:
    ...
    def __init__(
        self, api_key: str = '', retries=5, broker: Union[Broker, None] = None
    ):
    ...
    def __request(self, url):
        # print(url)
        if self._broker:
            params = {"api_key": self._broker.api_key, "url": url}
            url = self.__api_url_params(self._broker.host, params)
        try:
    ...
    @func_args_preprocessing
    def get_token_price(self, _id, contract_addresses, vs_currencies, **kwargs):
        ...
        api_url = '{0}simple/token_price/{1}'.format(self.api_base_url, _id)
        api_url = self.__api_url_params(api_url, kwargs)
        return self.__request(api_url)
    ...
    def get_coin_market_chart_range_by_id(
        self, id, vs_currency, from_timestamp, to_timestamp, **kwargs
    ):
        """Get historical market data include price, market cap, and 24h volume within a range of timestamp (granularity auto)"""

        api_url = (
            '{0}coins/{1}/market_chart/range?vs_currency={2}&from={3}&to={4}'.format(
                self.api_base_url, id, vs_currency, from_timestamp, to_timestamp
            )
        )
        api_url = self.__api_url_params(api_url, kwargs, api_url_has_params=True)
    ...
    @func_args_preprocessing
    def get_coin_market_chart_from_contract_address_by_id(
        self, id, contract_address, vs_currency, days, **kwargs
    ):
        """Get historical market data include price, market cap, and 24h volume (granularity auto) from a contract address"""

        api_url = (
            '{0}coins/{1}/contract/{2}/market_chart/?vs_currency={3}&days={4}'.format(
                self.api_base_url, id, contract_address, vs_currency, days
            )
        )
        api_url = self.__api_url_params(api_url, kwargs, api_url_has_params=True)
    ...
    @func_args_preprocessing
    def get_nfts_by_asset_platform_id_and_contract_address(
        self, asset_platform_id, contract_address, **kwargs
    ):
        """Get current data (name, price_floor, volume_24h ...) for an NFT collection. native_currency (string) is only a representative of the currency"""

        api_url = '{0}nfts/{1}/contract/{2}'.format(
            self.api_base_url, asset_platform_id, contract_address
        )
        api_url = self.__api_url_params(api_url, kwargs)
